...
import { Commander, ComponentInstance, ContentType, Injector, QueryStateType, Selection, Slot } from '@textbus/core'
...
function createViewer(content: string | HTMLElement, name: string): VNode {
  const card = h(ComponentCard, { content, name })
  return card
}
function createExample(injector: Injector, example: ComponentCreator, controller: Subject<void>): VNode {
...
export function componentsToolConfigFactory(injector: Injector): PopoverToolConfig {
