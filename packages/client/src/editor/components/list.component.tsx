...
export const listComponent = defineComponent({
  ...
  setup(data?: ComponentInitData<ListState>): ListComponentExtends {
    ...
    onContentInsert(ev => {
      ...
      if(ev.data.content === '    ' && selection.isCollapsed && selection.startOffset === 0) {
        ...
        if(!nextType) return
        const nextComponent = listComponent.createInstance(injector, {
          state: {
            tag: state.tag,
    ...
    onBreak(ev => {
      ...
      if(ev.target.isEmpty) {
        if(ev.target.parent?.parentComponent?.name === 'ListComponent') {
...
export const listComponentLoader: ComponentLoader = {
  ...
  read(element: HTMLElement, injector: Injector, slotParser: SlotParser): ComponentInstance {
    // console.log('listcomponent loader')
