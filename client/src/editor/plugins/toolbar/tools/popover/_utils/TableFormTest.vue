...
import { VNode, VNodeChild, h, onMounted, onUnmounted, reactive, ref, shallowRef } from 'vue'
...
const map = reactive(new Map<VNode, { row: number, col: number }>())
...
<template>
  <div class="image-form">
    <div ref="selector" class="textbus-toolbar-table-quick-selector">
      <component v-for="(item, index) in map" :key="index" :is="item[0]" @mouseover="handleMouseover(item)" />
    </div>
    <n-form ref="formRef" :model="model" :rules="rules" :show-require-mark="false">
      ...
      <n-button block @click="handleSubmit">确定</n-button>
    </n-form>
  </div>
</template>
